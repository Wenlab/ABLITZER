function mcdf=readOneFrame(fid,flag)
    ...
    while ~isEndOfFrame(tline)
        if isField(tline)
            ...
            switch fname
                ...
                case 'EyeOrientation'
                    [mcdf.EyeOrientation, tline]=getLeftRight(fid,tline);
                case 'CrossedAngle'
                    [mcdf.CrossedAngle tline]=getVal(fid,tline);
                case 'LEDIsOn'
                    [mcdf.LEDIsOn tline]=getVal(fid,tline);
                otherwise
        ...
        if isField(tline)
            ...
            switch fname
                ...
                case 'TimeStamp'
                    [mcdf.TimeStamp tline]=getVal(fid,tline);
                case 'BendingAngle'
                    [mcdf.BendingAngle tline]=getVal(fid,tline);
                otherwise
                    disp(['fname matched nothing: ',fname])
                    tline=fgets(fid);
            end
...
function [xy tline] = getLeftRight(fid,tline) 
% Get the left and right values from a field that has two subfields, x & y
